...
from src.models.models import Chat, Message
...
class LLM:
    ...
    def create_chat_if_not_exists(self, user_id, bot_id):
        ...
        chat = (
            self.database.query(Chat).filter_by(user_id=user_id, bot_id=bot_id).first()
        )
        print("CHAT Query result: ", chat)
        if chat is None:
            chat = Chat(id=str(uuid.uuid4()), user_id=user_id, bot_id=bot_id)
    ...
    def chat(self, message, role, user_id, bot_id) -> BaseMessage:
        ...
        with_message_history = RunnableWithMessageHistory(
            chain, self.get_session_history
        )
    ...
    def store_message(self, content, session_id, sender):
        ...
        try:
            message = Message(
                id=uuid.uuid4(), chat_id=session_id, sender=sender, content=content
            )
